...
impl BitmexExecutionClient {
    ...
    pub fn new(core: ExecutionClientCore, config: BitmexExecClientConfig) -> anyhow::Result<Self> {
        ...
        let submitter_config = SubmitBroadcasterConfig {
            ...
            timeout_secs: Some(config.http_timeout_secs),
            max_retries: Some(config.max_retries),
            retry_delay_ms: Some(config.retry_delay_initial_ms),
            retry_delay_max_ms: Some(config.retry_delay_max_ms),
            recv_window_ms: Some(config.recv_window_ms),
            max_requests_per_second: Some(config.max_requests_per_second),
            max_requests_per_minute: Some(config.max_requests_per_minute),
